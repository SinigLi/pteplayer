      ...
      myDisableRedraw(false)
{
    ...
    myDefaultPalette = &myLightPalette;
...
void ScoreArea::renderDocument(const Document &document)
{
    ...
    for (int i = 0; i < num_threads; ++i)
    {
        ...
        const int right = (i == num_threads - 1) ? myRenderedSystems.size()
                                                 : (i + 1) * work_size;
        auto run_fun = [&](int left, int right)
        {
        ...
        run_fun(left, right);
        //tasks.push_back(std::async(std::launch::async, run_fun, left, right));
    }
...
void ScoreArea::adjustScroll()
{
    if (myDocument->getCaret().isInPlaybackMode())
    {
        ...
        auto viewRect = transform().inverted().mapRect(this->rect());
        double vheight = viewRect.height();
        double vYvalue =
            myCaretPainter->getCurrentSystemRect().y()
            + myCaretPainter->getCurrentSystemRect().height()/2.0
            - vheight / 4.0;
        QPoint point(0, vYvalue);
...
void ScoreArea::scroolXToMiddle()
{
    auto viewRect = transform().inverted().mapRect(this->rect());
    double vwidth = viewRect.width();
    // qDebug()<<"vwidth"<<vwidth;
    double vXvalue =
        myCaretPainter->getCurrentSystemRect().x()
        + myCaretPainter->getCurrentSystemRect().width()/2.0
        -vwidth/2.0;
    // qDebug()<<"vXvalue"<<vXvalue;
    QPoint point(vXvalue,0 );
    point = transform().map(point);
    // qDebug()<<"point.x()"<<point.x();
    horizontalScrollBar()->setValue(point.x());
    // qDebug()<<"scroolXToMiddle"<<horizontalScrollBar()->minimum()+value;
}
...
QRectF
ScoreArea::getSysRect(int startI, int sysCount) const
{
    return myCaretPainter->getSystemRect(startI, sysCount);
}
QSizeF ScoreArea::getAvgSysSize() const
{
    if (!myCaretPainter) {
        return QSizeF();
    }
    return myCaretPainter->avgSysSize();
}


QRectF
ScoreArea::getStartInfoRect() const
{
    QRectF result;
    result = result.united(myScoreInfoBlock->sceneBoundingRect());
    result = result.united(myChordDiagramList->sceneBoundingRect());
    return result;
}
