      ...
      myVoices(new QButtonGroup(this))
{
    ...
    ui->speedSpinner->setMaximum(500);
    // ui->speedSpinner->setSuffix(QStringLiteral(" bpm"));
    ui->speedSpinner->setValue(120);
    ui->speedSpinner->setFocusPolicy(Qt::FocusPolicy::ClickFocus);
    ...
    ui->voiceLabel->setVisible(false);
    ui->voice1Button->setVisible(false);
    ui->voice2Button->setVisible(false);
    ui->locationLabel->setVisible(false);
    ui->line->setVisible(false);
    ui->metronomeToggleButton->setVisible(false);
    ui->countInToggleButton->setVisible(false);
}
...
void PlaybackWidget::reset(const Document &doc)
{
    ...
    if (doc.getScore().getBasicRateBp()!=0)
    {
        ui->speedSpinner->setValue(doc.getScore().getBasicRateBp());
    }
}

int
PlaybackWidget::getPlaybackSpeed(int orgBp) const
{
    double dv = (double)ui->speedSpinner->value();
    return (dv/(double)orgBp)*100;
}

void PlaybackWidget::setPlayBackSpeedBpm(int spBp)
{
    ui->speedSpinner->setValue(spBp);
}
