...
void center_symbol_vertically(QGraphicsItem& item, double y,double baseHeight)
{
    double sitemh = item.sceneBoundingRect().height();
    double itemheight = item.boundingRect().height();
	item.setY(y + 0.5 * (baseHeight - itemheight));
}
...
{
    myPlainTextFont.setPixelSize(8);
    myPlainTextFont.setStyleStrategy(QFont::PreferAntialias);
	myLyricTextFont.setPixelSize(14);
    myLyricTextFont.setStyleStrategy(QFont::PreferAntialias);
    mySymbolTextFont.setPixelSize(7);
                                          ...
                                          int systemIndex)
{
    ...
    for (const Staff &staff : system.getStaves())
    {
        ...
        LayoutConstPtr layout = std::make_shared<LayoutInfo>(location,&myViewOptions);
        ...
        height = drawSystemLyricsSymbols(location, *layout, height,myViewOptions.showNumberStuff(i));
        ...
        if (myViewOptions.showTabStuff(i) && !myViewOptions.showStdStuff(i))
        {
            height += layout->getTabStemExtraBeamHeight();
        }

        if (isFirstStaff)
            drawBarNumber(systemIndex, *layout);
        if (!myViewOptions.showStdStuff(i) 
            && !myViewOptions.showTabStuff(i))
        {
            ++i;
            continue;
        }
        ...
        if (myViewOptions.showStdStuff(i))
        {
            ...
            auto clef =
                new SimpleTextItem(staff.getClefType() == Staff::TrebleClef
        ...
        if (myViewOptions.showTabStuff(i) )
        {
            drawTabClef(LayoutInfo::CLEF_PADDING, *layout, location);
			std::map<int, QPointF> positionBottomPt;
			std::map<int, QPointF> positionTopPt;
            drawTabNotes(staff, layout, positionBottomPt, positionTopPt);

            if (!myViewOptions.showStdStuff(i))
            {
				drawTabStems(*layout, positionBottomPt);
				drawTilesByPos(myParentStaff, staff, *layout, positionTopPt, 4);
            }
        }
                                  ...
                                  std::map<int, QPointF>& positionTopPts)
{
    for (const Voice &voice : staff.getVoices())
    {
        for (const Position &pos : voice.getPositions())
		{
            ...
            tpPt.setX(location);
            bool bfirst = true;
            bool haddot = pos.hasProperty(Position::Dotted);
            bool haddotdot = pos.hasProperty(Position::DoubleDotted);
            for (const Note &note : pos.getNotes())
            {
                ...
                double midx = tabNote->sceneBoundingRect().center().x();
                double btny = tabNote->y()+2*layout->getTabLineSpacing();// tabNote->sceneBoundingRect().center().y();
                double topy = tabNote->y()-layout->getTabLineSpacing()*0.5;
                if (bfirst) {
					btPt.setY(btny);
                    tpPt.setY(topy);
                    bfirst = false;
			...
			positionBottomPt[pos.getPosition()] = btPt;
			positionTopPts[pos.getPosition()] = tpPt;
                                  ...
                                  const LayoutInfo& layout)
{
    ...
    const QChar arpeggioSymbol = (ushort)MusicFont::Vibrato;
    // const double symbolWidth = myMusicFontMetrics.width(arpeggioSymbol);
    const double symbolWidth = myMusicFontMetrics.boundingRect(arpeggioSymbol).width();
    ...
    const QChar arpeggioEnd = position.hasProperty(Position::ArpeggioUp) ?
                (ushort)MusicFont::ArpeggioUp : (ushort)MusicFont::ArpeggioDown;
                                       ...
                                       const LayoutInfo& layout)
{
    ...
    {
        int extheight = drawChordText(location, layout, height);
        height += LayoutInfo::SYSTEM_SYMBOL_SPACING + extheight;
        drawDividerLine(height);
    }
...
bool is_english(const QString& qstrSrc)
{
...
double
SystemRenderer::drawSystemLyricsSymbols(const ConstScoreLocation &location,
                                        const LayoutInfo &layout, double height,
                                        bool showNumberStuff)
{
    const System &system = location.getSystem();
    ...
    if(showNumberStuff)
    {
	    ...
	    const std::vector<StdNotationNote>& singNotes = layout.getStdNotationNotes();
	    std::map<int, size_t> pos2SingNodes;
	    int updotCount = 0, downDotCount = 0;

        for (size_t i = 0; i < singNotes.size(); ++i)
        {
            const StdNotationNote& onenode = singNotes.at(i);
            int level = onenode.singNodePitchLevel();
            if (level > updotCount)
            {
                updotCount = level;
            }
            if (level < downDotCount)
            {
                downDotCount = level;
            }
            pos2SingNodes[onenode.getPosition()] = i;
        }
        double perDotHeight = 4;
        double upDotHeight = updotCount * perDotHeight;
        double downDotHeight = std::abs(downDotCount * perDotHeight);
        double extBeamHeight = layout.getTabStemExtraBeamHeight(false);
        double gapBeamDist = 4;
	    double singNodeHeight = LayoutInfo::SYSTEM_SYMBOL_SPACING / 2.0;
	    double beamBotomY = height + singNodeHeight + upDotHeight + gapBeamDist + extBeamHeight;
        std::map<int, QPointF> positionBottomPt;
        std::map<int, QPointF> positionTopPts;
	    for (const Voice& voice : staff.getVoices())
	    {
		    for (const Position& pos : voice.getPositions())
		    {
                int posc = pos.getPosition();
                bool bIsEmptyPart = false;
                QString charstr = "";
                int level = 0;
                if (pos2SingNodes.find(posc) == pos2SingNodes.end()) {
                    bIsEmptyPart = true;
                    //continue;
                    charstr = "0";
                    if (pos.hasProperty(Position::Dotted)|| pos.hasProperty(Position::DoubleDotted))
                    {
                        charstr += " .";
			    ...
			    {
				    const StdNotationNote& singnode = singNotes[pos2SingNodes.at(posc)];
				    charstr = singnode.singNodeChars();
				    level = singnode.singNodePitchLevel();
                }
                if (level > 0)
                {
                    for (int i = 0; i < level; ++i)
				    {
                        ...
                        auto dotText =
                            new SimpleTextItem(dot, default_font,
                                TextAlignment::Baseline, QPen(myPalette.text().color()));
                        dotText->setX(layout.getPositionX(pos.getPosition())+2);
                        dotText->setY(height + (upDotHeight - perDotHeight * (i+1))+2);
					    //center_symbol_vertically(
					    //	*dotText, height + perDotHeight * i, perDotHeight);
                        dotText->setParentItem(myParentSystem);
                    }
                }
			    ConstScoreLocation item_location(location);
			    item_location.setPositionIndex(pos.getPosition());
			    auto text_item = new QGraphicsSimpleTextItem();
			    text_item->setFont(myPlainTextFont);
			    text_item->setText(charstr);
			    text_item->setBrush(myPalette.text());
			    text_item->setX(layout.getPositionX(pos.getPosition()));
                center_symbol_vertically(
				    *text_item, height + upDotHeight, singNodeHeight);
			    //curTextItems[i].push_back(text_item);
			    double midx = text_item->sceneBoundingRect().center().x();
                if (!bIsEmptyPart)
			    {
				    double botm = text_item->sceneBoundingRect().bottom();
				    double topm = text_item->sceneBoundingRect().top();
				    positionBottomPt[pos.getPosition()].setX(midx);
				    positionBottomPt[pos.getPosition()].setY(botm + gapBeamDist);
				    if (positionTopPts.find(pos.getPosition()) == positionTopPts.end())
				    {
					    positionTopPts[pos.getPosition()].setX(midx);
					    positionTopPts[pos.getPosition()].setY(topm - level * perDotHeight);
				    }
				    else
				    {
					    if (positionTopPts[pos.getPosition()].y() > topm)
					    {
						    positionTopPts[pos.getPosition()].setY(topm);
					    }
				    }
                }
                else
                {
                    QPainterPath subBeamPath = gen_single_beam_path(beamBotomY, midx,pos.getDurationType(),4);

				    auto beams = new QGraphicsPathItem(subBeamPath);
				    beams->setPen(QPen(myPalette.text().color(), 1, Qt::SolidLine, Qt::RoundCap));
				    beams->setParentItem(myParentSystem);
                }
			    text_item->setParentItem(myParentSystem);
                if (level < 0)
			    {
                    double downStartHeight = height + upDotHeight + singNodeHeight + extBeamHeight+ gapBeamDist*2;
				    for (int i = 0; i < std::abs(level); ++i)
				    {
					    ...
					    dotText->setY(downStartHeight + perDotHeight * i);
					    //center_symbol_vertically(
					    //	*dotText, downStartHeight + perDotHeight * i, perDotHeight);
                        dotText->setParentItem(myParentSystem);
				    }
                }
		    }
        ...
        drawStems(myParentSystem, layout, positionBottomPt, beamBotomY,true,1, singNodeHeight / 3,true);
        drawTilesByPos(myParentSystem, staff, layout, positionTopPts, 4);
	    height = beamBotomY + gapBeamDist +downDotHeight;
	}
	...
	for (const Voice& voice : staff.getVoices())
	{
		for (const Position& pos : voice.getPositions())
		{
			rowcount = std::max(pos.maxLyricRow(), rowcount);
		}
	}
    std::vector< std::list< QGraphicsSimpleTextItem* > > curTextItems;
    curTextItems.resize(rowcount);
    for (const Voice &voice : staff.getVoices())
    {
        for (const Position &pos : voice.getPositions())
        {
            ...
            {
                QString contents =
                    QString::fromStdString(pos.gerLyricRow(i));
                if (contents.isEmpty())
                {
                    continue;
                }
                ...
                item_location.setPositionIndex(pos.getPosition());
                // auto group =
                //    new ClickableGroup(ScoreArea::tr("Double-click to edit
                //    text."),
                //                       myScoreArea->getClickEvent(),
                //                       item_location, ScoreItem::TextItem);

                // Note: the SimpleTextItem class is not used here since
                // multi-line support is needed.
                auto text_item = new QGraphicsSimpleTextItem();
                text_item->setFont(myLyricTextFont);
                text_item->setText(contents);
                text_item->setBrush(myPalette.text());
                text_item->setX(layout.getPositionX(pos.getPosition()));
                centerSymbolVertically(
                    *text_item, height + i * LayoutInfo::SYSTEM_SYMBOL_SPACING);
                curTextItems[i].push_back(text_item);
                //text_item->setParentItem(myParentSystem);
            }
        }
    }
    if (!curTextItems.empty())
	{//
		auto group = new QGraphicsItemGroup();
        for (auto onerowLyxItems : curTextItems)
        {
            if (onerowLyxItems.empty()) {
                continue;
			}
            auto get_gap_dist = [this](const QGraphicsSimpleTextItem* text0, const QGraphicsSimpleTextItem* text1) {
                QString onetext = text0->text();
                QString towtext = text1->text();
                if (is_english(onetext) && is_english(towtext))
				{
					double basegapdist = myLyricTextFont.pixelSize() / 2;
                    return basegapdist;
                }
                return 1.0;
            };
            auto subGroup = new QGraphicsItemGroup();
            double lastRight = 0;
            {
				auto it = onerowLyxItems.begin();
				subGroup->addToGroup(*it);
				auto lastItem = *it;
				lastRight = lastItem->sceneBoundingRect().right();

				++it;
				for (; it != onerowLyxItems.end(); ++it)
				{
					auto oneitem = *it;
					//oneitem->sceneBoundingRect().right();
					double curleft = oneitem->sceneBoundingRect().left();
					if (curleft < lastRight)
					{
						double gapdist = get_gap_dist(lastItem, oneitem);
						double xmv = lastRight - curleft;
						oneitem->setX(oneitem->x() + xmv + gapdist);
					}
					lastRight = oneitem->sceneBoundingRect().right();
					lastItem = oneitem;
					subGroup->addToGroup(oneitem);
				}
            }
            if (lastRight > layout.STAFF_WIDTH)
            {
                double xmv = lastRight - layout.STAFF_WIDTH;
                //反向再处理一遍
                auto rit = onerowLyxItems.rbegin();
                auto lastitem = *rit;
                lastitem->setX(lastitem->x() - xmv);
                double lastLeft = lastitem->sceneBoundingRect().left();
				++rit;
                for (; rit != onerowLyxItems.rend(); ++rit)
                {
					auto oneitem = *rit;
					//oneitem->sceneBoundingRect().right();
					double curRight = oneitem->sceneBoundingRect().right();
					if (curRight > lastLeft)
					{
						double gapdist = get_gap_dist(lastitem, oneitem);
						double xmv = curRight - lastLeft;
						oneitem->setX(oneitem->x() - xmv - gapdist);
					}
                    lastLeft = oneitem->sceneBoundingRect().left();
                    lastitem = oneitem;
                }

                if (lastLeft < 0)
                {
                    double absDist =std::abs(lastLeft);
                    QTransform tr = subGroup->transform();
                    //先平移到原点进行缩放，然后重新平移回去
                    QPointF mid = subGroup->sceneBoundingRect().center();
                    subGroup->moveBy(-mid.x(), -mid.y());

                    double width = subGroup->sceneBoundingRect().width();
                    double scale = layout.STAFF_WIDTH / width;
					subGroup->setScale(scale);
					subGroup->moveBy(mid.x(), mid.y());

                    //double x = subGroup->x();
                    //double y = subGroup->y();
                    subGroup->setX(subGroup->x() + absDist*scale);
                    //subGroup->setScale(scale);
                }
            }
    ...
    return height + rowcount * LayoutInfo::SYSTEM_SYMBOL_SPACING;
}

void
SystemRenderer::drawDividerLine(double y)
{
                                 ...
                                 const LayoutInfo &layout, double height)
{
    ...
    for (const TempoMarker &tempo : system.getTempoMarkers())
    {
        ...
        else
        {
            ...
            auto pixmap = new QGraphicsPixmapItem(image.scaled(
                fm.boundingRect(imageSpacing).width(), NOTE_HEIGHT,
                Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
            // pixmap->setX(fm.width(text));
            pixmap->setX(fm.boundingRect(text).width());
            ...
            if (tempo.getTripletFeel() != TempoMarker::NoTripletFeel)
            {
                ...
                // pixmap->setX(fm.width(text));
                pixmap = new QGraphicsPixmapItem(image.scaled(
                    fm.boundingRect(imageSpacing).width(), 21,
                    Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
                pixmap->setX(fm.boundingRect(text).width());
                centerSymbolVertically(*pixmap, height);
                group->addToGroup(pixmap);
                              ...
                              const LayoutInfo &layout, double height)
{
    ...
    for (const ChordText &chord : location.getSystem().getChords())
    {
        ...
        group->setParentItem(myParentSystem);
        const ChordDiagram * diagram=nullptr;
        if (!myScore.getFirstChordDriagram(chord.getChordName(), diagram))
        {
            continue;
        }
        DiagramSizeInfo diasize;
        diasize.setHeightByScale(LayoutInfo::SYSTEM_PER_CHORD_DIAGRAM_SPACING*2);
        auto diagram_item =
            new ChordDiagramPainter(*diagram, myPalette.text().color(), diasize);
        const QSizeF size = diagram_item->boundingRect().size();
        diagram_item->setPos(x-size.width()/2.0,textItem->y());
        extheight = LayoutInfo::SYSTEM_PER_CHORD_DIAGRAM_SPACING;
        //size.height();
        
        //textItem->setPos(
        //    0.5 * (size.width() - textItem->boundingRect().width()), 0);
...
//
//int
//SystemRenderer::drawLyricsItems(const ConstScoreLocation &location,
//                                const LayoutInfo &layout, double height)
//{
//    int maxrowcount = 0;
//
//    for (const TextItem &text :
//         location.getSystem().staffLyrics().at(location.getStaffIndex()))
//    {
//        const QString &contents = QString::fromStdString(text.getContents());
//        maxrowcount = std::max(contents.split("\r\n").size(), maxrowcount);
//        ConstScoreLocation item_location(location);
//        item_location.setPositionIndex(text.getPosition());
//        // auto group =
//        //    new ClickableGroup(ScoreArea::tr("Double-click to edit text."),
//        //                       myScoreArea->getClickEvent(), item_location,
//        //                       ScoreItem::TextItem);
//
//        // Note: the SimpleTextItem class is not used here since multi-line
//        // support is needed.
...
void SystemRenderer::drawSymbolsBelowTabStaff(const LayoutInfo &layout)
{
    for (const SymbolGroup &symbolGroup : layout.getTabStaffBelowSymbols())
    {
        ...
        case SymbolGroup::PickStrokeDown:
			...
			renderedSymbol = createPickStroke(QChar(MusicFont::PickStrokeUp),180,QPointF(2,-3));
            break;
                                         ...
                                         const LayoutInfo &layout)
{
    for (const SymbolGroup &symbolGroup : layout.getTabStaffAboveSymbols())
    {
        ...
        case SymbolGroup::Vibrato:
            renderedSymbol = drawContinuousFontSymbols((ushort)MusicFont::Vibrato,
                                                       width);
            break;
        case SymbolGroup::WideVibrato:
            renderedSymbol = drawContinuousFontSymbols((ushort)MusicFont::WideVibrato,
                                                       width);
            break;
                                                         ...
                                                         int width)
{
    ...
    const double symbolWidth = QFontMetricsF(font).boundingRect(symbol).width();
                                ...
                                const LayoutInfo &layout)
{
    ...
    for (const StdNotationNote &note : notes)
    {
        ...
        const double note_head_width = fm->boundingRect(note_head_char).width();

        const QString accidental_text = note.getAccidentalText();
        // const double accidental_width = fm->width(accidental_text);
        const double accidental_width = fm->boundingRect(accidental_text).width();
        ...
        if (note.isDotted() || note.isDoubleDotted())
        {
            ...
            const double dotX = fm->boundingRect(note_text).width() + 2;
...
void SystemRenderer::drawTabStems(const LayoutInfo& layout,
	std::map<int, QPointF>& positionBottomPt)
{
    int extractHeight = getTabStemExtraBeamHeight(layout);
    drawStems(myParentStaff,layout,positionBottomPt, layout.getBottomTabLine() + extractHeight, false);
	...
	//{
	//	const std::vector<BeamGroup>& beamGroups = layout.getBeamGroups(v);
	//	std::vector<NoteStem> stems = layout.getNoteStems(v);
 //       for (NoteStem& stem : stems)
 //       {
	//		stem.setStemType(NoteStem::StemDown);
	//		stem.setBottom(layout.getBottomTabLine()+extractHeight);
 //           if (positionBottomPt.find(stem.getPositionIndex()) != positionBottomPt.end())
 //           {
 //               QPointF pt = positionBottomPt.at(stem.getPositionIndex());
 //               stem.setTop(pt.y());
 //               stem.setX(pt.x());
 //           }
 //           else
 //           {
	//			stem.setTop(layout.getTopTabLine());
 //           }
 //       }
	//	//if(0)
	//	for (BeamGroup group : beamGroups)
	//	{
 //           group.setStemType(NoteStem::StemDown);
	//		group.drawStems(myParentStaff, stems, myMusicNotationFont,
	//			myPalette.text().color(), layout);
	//	}
...
void SystemRenderer::drawStems(QGraphicsItem* parent,const LayoutInfo& layout,
    std::map<int, QPointF>& positionBottomPt,
    double stemBottomY, bool drawBeamOnly, double beamLineWidth, 
    double extBeam, bool singleBeamToLine)
{
	//int extractHeight = getTabStemExtraBeamHeight(layout);
	for (int v = 0; v < Staff::NUM_VOICES; ++v)
	{
		...
		{
			group.setStemType(NoteStem::StemDown);
			group.drawStems(parent, stems, myMusicNotationFont,
				myPalette.text().color(), layout, drawBeamOnly, 
                beamLineWidth, extBeam, singleBeamToLine);
		}
...
void SystemRenderer::drawTilesByPos(QGraphicsItem* parent, const Staff& staff, 
    const LayoutInfo& layout, const std::map<int, QPointF>& positionTopPt,double baseHeiht)
{
    ...
    for (int v = 0; v < Staff::NUM_VOICES; ++v)
	{
		...
		for (const StdNotationNote& note : notes)
		{
            ...
            if (positionTopPt.find(pos) == positionTopPt.end()) {
                continue;
            }
            QPointF topPt = positionTopPt.at(pos);
			const NoteStem& stem = stems.at(
				ScoreUtils::findIndexByPosition(voice.getPositions(), pos));
			const int prevPos = *note.getTie();
            if (positionTopPt.find(prevPos) == positionTopPt.end()) {
                continue;
            }
            QPointF preTopPt = positionTopPt.at(prevPos);
			// Draw to the centre of each position.
            const double x = topPt.x();// layout.getPositionX(note.getPosition()) +
				//0.5 * layout.getPositionSpacing();
            const double prevX = preTopPt.x();
            ...
            const double y = topPt.y()- baseHeiht;
			...
			arc->setPos(prevX, y);
			arc->setPen(QPen(myPalette.text().color()));
			arc->setParentItem(parent);
		}
    }

}

int SystemRenderer::getTabStemExtraBeamHeight(const LayoutInfo& layout) const
{
    return layout.getTabStemExtraBeamHeight();
	/*int PER_BEAM_HEIGHT = 3;
	...
	for (int v = 0; v < Staff::NUM_VOICES; ++v)
	{
		...
		{
			int ebcount = getNumExtraBeams(stem);
			if (ebcount < 1) {
				continue;
			}
			if (maxHeightCount < ebcount)
			{
				maxHeightCount = ebcount;
			}
		}
	}
	return maxHeightCount * PER_BEAM_HEIGHT + PER_BEAM_HEIGHT*4;*/
}
                                         ...
                                         const std::vector<NoteStem> &stems)
{
    ...
    for (const IrregularGrouping &group : voice.getIrregularGroupings())
    {
        ...
        const double textWidth = fm.boundingRect(text).width();
...
SystemRenderer::drawRest(const Position &pos, double x,
                         const LayoutInfo &layout, bool drawByStd)
{
    // Position it in the middle of the staff.
    QFont font = MusicFont::getFont(24);
    if (!drawByStd)
    {
        font = MusicFont::getFont(16);
    }
    double y = layout.getStdNotationLine(3);
    if (!drawByStd)
    {
        y = layout.getTabLine(3);
    }
    QChar symbol;
    switch (pos.getDurationType())
    {
        case Position::WholeNote:
            symbol = (ushort)MusicFont::WholeRest;
            y = layout.getStdNotationLine(2);
            if (!drawByStd)
            {
                y = layout.getTabLine(2);
            }
            break;
        case Position::HalfNote:
            symbol = (ushort)MusicFont::HalfRest;
            break;
        case Position::QuarterNote:
            symbol = (ushort)MusicFont::QuarterRest;
            break;
        case Position::EighthNote:
            symbol = (ushort)MusicFont::EighthRest;
            break;
        case Position::SixteenthNote:
            symbol = (ushort)MusicFont::SixteenthRest;
            break;
        case Position::ThirtySecondNote:
            symbol = (ushort)MusicFont::ThirtySecondRest;
            break;
        case Position::SixtyFourthNote:
            symbol = (ushort)MusicFont::SixtyFourthRest;
            break;
    }
    ...
    const QChar dot = (ushort)MusicFont::Dot;
    const double dotX = 0.4 * font.pixelSize();
    // Position just below second line of staff.
	double dotY = layout.getStdNotationSpace(2);
	if (!drawByStd)
	{
		dotY = (layout.getTabLine(2)+ layout.getTabLine(3))/2.0;
	}
