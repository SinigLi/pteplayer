      ...
      myTie(tie)
{
    ...
    case Position::WholeNote:
        myNoteHeadSymbol = (ushort)MusicFont::WholeNote;
        break;
    case Position::HalfNote:
        myNoteHeadSymbol = (ushort)MusicFont::HalfNote;
        break;
    default:
        myNoteHeadSymbol = (ushort)MusicFont::QuarterNoteOrLess;
        break;
    }
        ...
        note.hasArtificialHarmonic())
    {
        myNoteHeadSymbol = (pos.getDurationType() <= Position::HalfNote)
                               ? (ushort)MusicFont::HarmonicNoteHeadOpen
                               : (ushort)MusicFont::HarmonicNoteHeadFull;
    }
    else if (note.hasProperty(Note::Muted))
        myNoteHeadSymbol = (ushort)MusicFont::MutedNoteHead;
	...
	unsigned int baseSingPitch = tuning.getSingNodeBase()+ key.getOffsetKey();
	int diff = (pitch - baseSingPitch);
	mySingNodeUpDownDotCount = std::floor(diff / 12.0);
	int mode = diff % 12;
    if (mode < 0)
    {
        mode += 12;
    ...
    if (isDotted()||isDoubleDotted())
    {
		mySingNode +=QString::fromStdWString(L" .");
		if (isDoubleDotted())
		{
			mySingNode += QString::fromStdWString(L".");
		}
        ...
        for (const Barline &bar : system.getBarlines())
        {
                     ...
                     nextBar->getPosition()))
            {
                ...
                for (const Note &note : pos.getNotes())
                {
                    ...
                    noteHeadWidth = fm->boundingRect(stdNote.getNoteHeadSymbol()).width();
                }
