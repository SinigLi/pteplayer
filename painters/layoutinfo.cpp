...
const double LayoutInfo::STAFF_WIDTH = 740;
const int LayoutInfo::NUM_STD_NOTATION_LINES = 5;
const double LayoutInfo::STD_NOTATION_LINE_SPACING = 5;
const double LayoutInfo::STAFF_BORDER_SPACING = 8;
const double LayoutInfo::CLEF_PADDING = 2;
const double LayoutInfo::BAR_NUMBER_PADDING = 2;
const double LayoutInfo::ACCIDENTAL_WIDTH = 4;
const double LayoutInfo::CLEF_WIDTH = 17;
const double LayoutInfo::SYSTEM_SYMBOL_SPACING = 17;
const double LayoutInfo::SYSTEM_PER_CHORD_DIAGRAM_SPACING = 18;
const double LayoutInfo::MIN_POSITION_SPACING = 2;
const double LayoutInfo::TAB_SYMBOL_SPACING = 6;
const double LayoutInfo::DEFAULT_POSITION_SPACING = 14;
const double LayoutInfo::IRREGULAR_GROUP_HEIGHT = 5;
const double LayoutInfo::IRREGULAR_GROUP_BEAM_SPACING = 2;

LayoutInfo::LayoutInfo(const ConstScoreLocation &location,
                       const ViewOptions *viewOpt)
    : myLocation(location),
...
void
LayoutInfo::setViewOpts(const ViewOptions *viewOpt)
{
    mViewOpt = viewOpt;
}
...
double LayoutInfo::getSystemSymbolSpacing() const
{
    ...
    {
        height += SYSTEM_SYMBOL_SPACING+SYSTEM_PER_CHORD_DIAGRAM_SPACING;
    }
...
double LayoutInfo::getStaffHeight() const
{
    ...
    double height = 0.0;
    if (mViewOpt->showStdStuff(myLocation.getStaffIndex()))
    {
        height += myStdNotationStaffAboveSpacing +
                  myStdNotationStaffBelowSpacing +
                  STD_NOTATION_LINE_SPACING * (NUM_STD_NOTATION_LINES - 1) +
                  2 * STAFF_BORDER_SPACING;
    }
    if (mViewOpt->showTabStuff(myLocation.getStaffIndex()))
    {
        height += myTabStaffAboveSpacing + myTabStaffBelowSpacing +
			(getStringCount() - 1) * getTabLineSpacing();
		height += 2 * STAFF_BORDER_SPACING;
        //if (!mViewOpt->showStdStuff(myLocation.getStaffIndex()))
        //{
        //    //height += getTabStemExtraBeamHeight() ;
        //}
    }
    return height;
}
...
double LayoutInfo::getTabStemExtraBeamHeight(bool containStem) const
{
	...
	{
		const std::vector<BeamGroup>& beamGroups = getBeamGroups(v);
		std::vector<NoteStem> stems = getNoteStems(v);
    ...
    double extheight = PER_BEAM_HEIGHT * 6;
    if (!containStem)
    {
        extheight = 0.0;
    }
	return maxHeightCount * PER_BEAM_HEIGHT + extheight;
}
...
bool
LayoutInfo::isShowTabStuff() const
{
    return mViewOpt->showTabStuff(myLocation.getStaffIndex());
}


bool
LayoutInfo::isShowStdStuff() const
{
    return mViewOpt->showStdStuff(myLocation.getStaffIndex());
}
