                       ...
                       bool allow_count_in)
{
    ...
    //emit playbackSystemChanged(current_location.getSystem());
    ...
    for (const MidiEvent &event : events)
    {
        ...
        auto error_correction = sleep_duration<clock_drift?sleep_duration:clock_drift;
...
MidiPlayer::playScore(const ConstScoreLocation &start_score_location, int speed)
{
    ...
    //bool acn = true;
    //if(start_location.getSystem()!=0
    //    &&start_location.getPosition()!=0)
    //{
    //    acn = false;
    //}

    if (playEvents(file, score, start_location,true))
