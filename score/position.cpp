...
size_t
Position::maxLyricRow() const
{
    size_t r = 0;
    for (size_t i  =0;i<myLyrics.size();++i)
    {
        if (!myLyrics.at(i).empty())
        {
            r = i+1;
            //myLyrics.at(i);
        }
...
std::string
Position::gerLyricRow(size_t r) const
{
    if (myLyrics.size() <= r)
    {
        return "";
    }
    return myLyrics.at(r);
}
