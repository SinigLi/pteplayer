...
{
public:
    RepeatedSection(const SystemLocation &startBar = SystemLocation());
    ...
    bool isLastRepeatTrun() const;

    void setLocation(SystemLocation loc)
    {
        myStartBarLocation = loc;
    }
    void setBaseEndLocation(SystemLocation loc)
    {
        myBaseEndLocation = loc;
    }
    void setLastEndLocation(SystemLocation loc)
    {
        myLastEndLocation = loc;
    }
    std::map<int, SystemLocation> &alterNateEndding()
    {
        return myAlternateEndings;
    }
    const std::map<int, SystemLocation> &alterNateEndding()const
    {
        return myAlternateEndings;
    }
    std::map<int, SystemLocation> &alterNateStartting()
    {
        return myAlyernateStatings;
    }
    const std::map<int, SystemLocation> &alterNateStartting() const
    {
        return myAlyernateStatings;
    }
    void setMaxRepeatCount(int count)
    {
        myMaxRepeatRound = count;
    }

private:
    SystemLocation myStartBarLocation;
    //std::map<SystemLocation, int> myRepeatEndBars;
    SystemLocation myBaseEndLocation;
    SystemLocation myLastEndLocation;
    std::map<int, SystemLocation> myAlternateEndings;
    std::map<int, SystemLocation> myAlyernateStatings;
    int myMaxRepeatRound = 1;
    int myActiveRepeat;
};
