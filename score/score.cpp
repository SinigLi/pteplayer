...
int
Score::getBasicRateBp() const
{
    for (auto onesys:mySystems)
    {
        for (auto onetmpo:onesys.getTempoMarkers())
        {
            return onetmpo.getBeatsPerMinute();
        }
    }
    return 0;
}
...
int
Score::getRateBpByPos(int sys, int pos) const
{
    if (mySystems.size() <= sys)
    {
        return 0;
    }
    int lastRate = getBasicRateBp();
    for (size_t i = 0; i < mySystems.size();++i)
    {
        const System &system = mySystems.at(i);
        if (i>sys)
        {
            return lastRate;
        }
        for (auto onetmpo : system.getTempoMarkers())
        {
            if (i==sys && onetmpo.getPosition()>pos)
            {
                return lastRate;
            }
            lastRate = onetmpo.getBeatsPerMinute();
        }
    }
    return lastRate;
}
...
int
Score::getBarByPos(int sys, int pos) const
{
    if (mySystems.size() <= sys)
    {
        return 0;
    ...
    int lastBar = -1;
    const System &system = mySystems.at(sys);
    for (auto onebar : system.getBarlines())
    {
        if (onebar.getPosition() > pos)
        {
            return lastBar;
        }
        //lastBar = lastBar;
...
bool
Score::getFirstChordDriagram(const ChordName &name,
                             const ChordDiagram *& chorddiag) const
{

    for (const ChordDiagram &diagram : getChordDiagrams())
    {
        if (diagram.getChordName()==name)
        {
            chorddiag = &diagram;
            return true;
        }
