              ...
              int bar_end)
{
    ...
    for (auto &&player : score.getPlayers())
    {
        ...
        //system.staffLyrics().push_back(std::vector<TextItem>());
    }
    ...
    for (int bar_idx = bar_begin; bar_idx < bar_end; ++bar_idx)
    {
        ...
        for (int staff_idx = 0; staff_idx < num_staves; ++staff_idx)
        {
            ...
            std::vector<TextItem> curStaffLyrics;
            ...
            for (int voice_idx = 0; voice_idx < num_voices; ++voice_idx)
            {
                ...
                for (int gp_beat_idx : gp_voice.myBeatIds)
                {
                    ...
                    //system.insertTextItem(TextItem(voice_pos, "a"));
                    //if (!gp_beat.myFreeText.empty())
                    //{
                    //    system.insertTextItem(
                    //        TextItem(voice_pos, gp_beat.myFreeText));
                    //}
                    std::vector<std::string > lyricstrs;
                    //std::string spltstr = "";
                    for (size_t lri = 0; lri < gp_beat.myLyrics.size(); ++lri)
                    {
                        const std::string &one = gp_beat.myLyrics.at(lri);
                        //if (one.empty())
                        //{
                        //    continue;
                        //}
                        lyricstrs.push_back(one);
                        //if (!lyricstr.empty())
                        //{
                        //    lyricstr += "\r\n";
                        //}
                    ...
                    //if (!lyricstr.empty())
                    //{
                    //    curStaffLyrics.push_back(TextItem(voice_pos, lyricstr));
                    //    //system.insertTextItem();
                    //}
                    if (!gp_beat.myFreeText.empty())
                    {
            ...
            //for (auto & oneLyric: curStaffLyrics)
            //{
            //    system.staffLyrics()[staff_idx].push_back(std::move(oneLyric));
            //}
            //system.staffLyrics()[staff_idx] =std::move(curStaffLyrics);
        }
