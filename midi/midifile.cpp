...
static void
checkRepeatSegnoMark(RepeatController &repeat_controller, 
                     SystemLocation location)
{
    repeat_controller.checkForSegnoMark(location);
}
...
void MidiFile::load(const Score &score, const LoadOptions &options)
{
    ...
    while (location.getSystem() < static_cast<int>(score.getSystems().size()))
    {
        ...
        if (location.getSystem() != system_index)
        {
            ...
            location.setPosition(current_bar.getPosition());
        }
        checkRepeatSegnoMark(repeat_controller,  location);
